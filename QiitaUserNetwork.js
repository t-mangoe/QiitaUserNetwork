...
function AddUsersToNetwork(data, centerUserId, followerFlag) {
    ...
    $.each(data, function (index, val) {
        ...
        if (!(addedUserId in nodeIdMap)) {
            ...
            nodeIdNumber++;
            var node = {
                id: nodeIdMap[addedUserId],
                label: addedUserId,
                shape: 'image',
                image: val.profile_image_url,
                mass: mass
            };
        ...
        if (followerFlag) {
            edges.add({
                from: nodeIdMap[addedUserId],
                to: nodeIdMap[centerUserId],
                arrows: 'to'
            });
        } else {
            edges.add({
                from: nodeIdMap[centerUserId],
                to: nodeIdMap[addedUserId],
                arrows: 'to'
            });
        }
    });
